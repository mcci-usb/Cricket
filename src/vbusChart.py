...
class DataThread(threading.Thread):
    """
    Data start with in the thread.
    The DataThread navigate to Super a window ran with in the thread.
    """
    def __init__(self, parent):
        """
        VBUS VI plot works through Data thread
        Args:
    ...
    def run(self) -> None:
        ...
        """
        while self.wait:
          self.window.push_data()
          time.sleep(0.1)

    def stop(self):
        """
        status returns the stop.
        Args:
            self: The self parameter is a reference to the current 
            instance of the class,and is used to access variables
            that belongs to the class.
...
class ChartThread(threading.Thread):
    """
    A class Chart thread works with init method
    """
    def __init__(self, parent):
        """
        Plot chart threading running under a threading
        Args:
            self: The self parameter is a reference to the current 
            instance of the class,and is used to access variables
            that belongs to the class.
        ...
        Returns:
            None
        """
        super(ChartThread, self).__init__()
        self.window = parent
        self.wait = True
   
    def run(self) -> None:
       """
        start the running for volts and amps graph
        Args:
            self: The self parameter is a reference to the current 
            instance of the class,and is used to access variables
            that belongs to the class.
        ...
        """
       while self.wait:
          if self.window.run_flg:
              if(len(self.window.xd) > 1):
...
class VbusChart(wx.Frame):
 ...
 def __init__(self, parent, top):
   ...
   self.data_thread.daemon = False
   self.chart_thread = ChartThread(self)
   self.chart_thread.daemon = False
