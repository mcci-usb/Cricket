...
class MidPanel(wx.Panel):
    ...
    def __init__(self, parent, top, portno):
        ...
        self.loopPan = loopWindow.LoopWindow(self, self.parent)
        self.autoPan = autoWindow.AutoWindow(self, self.parent)

        self.swobjmap = {"3141": Dev3141Window, "3201": Dev3201Window, "2301": Dev2301Window, "2101": Dev2101Window}
        
        # self.dev3141Pan = dev3141Window.Dev3141Window(self, self.parent, self.portno)
        # self.dev3201Pan = dev3201Window.Dev3201Window(self, self.parent, self.portno)
        # self.dev2101Pan = dev2101Window.Dev2101Window(self, self.parent, self.portno)
        # self.dev2301Pan = dev2301Window.Dev2301Window(self, self.parent, self.portno)

        self.swobj = {"NA": Dev2101Window(self, self.parent, self.portno)}
        ...
        # self.devdict = {}
        # self.devdict["3141"] = self.dev3141Pan
        # self.devdict["3201"] = self.dev3201Pan
        # self.devdict["2101"] = self.dev2101Pan
        # self.devdict["2301"] = self.dev2301Pan


        
        # Creating Sizers
        self.vboxsw = wx.BoxSizer(wx.VERTICAL)
        self.vboxdl = wx.BoxSizer(wx.VERTICAL)
        swval = list(self.swobj.values())
        self.vboxsw.Add(swval[0], 0, wx.EXPAND)
        self.vboxsw.Add(0, 10, 0)
        ...
        self.vboxdl.Add(self.vboxsw, 1, wx.EXPAND)
    ...
    def hide_switch(self):
        ...
        swval = list(self.swobj.values())
        self.vboxdl.Hide(swval)
        self.Layout()

    def remove_switch(self):
        self.clear_sw_window()
        self.Layout()

    def clear_sw_window(self):
        self.vboxsw.Clear(True)
        self.swobj.clear()


    def add_switch(self, swlist):
        self.clear_sw_window()
        port = list(swlist.keys())
        swname = list(swlist.values())
        self.portno = port[0]
        self.swobj[self.portno] = self.swobjmap[swname[0]](self, self.parent, self.portno)
        self.vboxsw.Add(self.swobj[self.portno], 0, wx.EXPAND)
        self.vboxsw.Add(0, 10, 0)
        self.Layout()


    def show_switch(self, swlist):
        print("I am MidPanel show switch")
        print(swlist)
        port = list(swlist.keys())
        swid = list(swlist.values())
        self.vboxdl.Hide(self.dev2301Pan)
        self.vboxdl.Hide(self.dev3201Pan)
        self.vboxdl.Hide(self.dev3141Pan)
        self.vboxdl.Hide(self.dev2101Pan)

        self.vboxdl.Show(self.devdict[swid[0]])
        self.devdict[swid[0]].update_cport(port[0])
    ...
    def update_controls(self, mode):
        ...
        key = list(self.swobj.keys())[0]
        self.swobj[key].update_controls(mode)
