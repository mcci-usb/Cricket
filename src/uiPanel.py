...
class UiPanel(wx.Panel):
    ...
    def __init__(self, parent):
        ...
        # self.swuiobj = {}
        # self.swmap = {"3141": Dev3141Window, "3201": Dev3201Window, "2101": Dev2101Window, "2301": Dev2301Window}

        self.Layout()

    def StopSequence(self, event):
        # title = ("Fault Received!")
        # msg = (event.data)
        # dlg = wx.MessageDialog(self, msg, title, wx.OK)
        # dlg.ShowModal()
        if(event.data != None):
            self.cpanel.PrintLog("\n"+event.data)
    ...
    #     print("I am UiPanel, Add Switches: ",swlist)
    #     for key, value in swlist.items():
    #         #self.swuiobj.append(self.swmap[value](self,self.parent, key))
    #         self.swuiobj[key] = self.swmap[value](self,self.parent, key)

    #     if len(swlist) > 1:
    #         self.lpanel.add_switches(self.swuiobj)
    #         self.lpanel.Show()
    #         self.cpanel.hide_switch()
    #     else:
    #         self.lpanel.Hide()
    #         self.cpanel.show_switch(swlist)
    #     self.Layout()

    def add_switches(self, swlist):
        print("I am UiPanel, Add Switches: ",swlist)
        if len(swlist) > 1:
            self.cpanel.remove_switch()
            self.lpanel.add_switches(swlist)
            self.lpanel.Show()
        else:
            self.lpanel.Hide()
            self.cpanel.add_switch(swlist)
        self.Layout()
    ...
    def port_on(self, swkey, port, stat, swcnt):
        ...
        """
        if(swcnt > 1):
            print("Multi Switch connected, look into LPanel")
            print(swkey, port, stat)
            self.lpanel.port_on(swkey, port, stat)
        else:
            print("Single SW connected look into CPanel")
            self.cpanel.port_on(swkey, port, stat)
    ...
    def WhenErrorOccurred(self):
        print("Iam UiPanel ------> Error Occurred")
